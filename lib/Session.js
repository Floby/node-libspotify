 ...
 */
Session.prototype.login = function login(login, password, remember_me) {
    ...
    if('boolean' != typeof remember_me) throw new TypeError('remember_me should be a boolean');
    b.session_login(this._sp_session, login, password, remember_me);
};

/**
 * try to relogin with the stored credentials
 * when this request is processed by libspotify, the 'login' event is
 * fired with an error or null as first argument
 */
Session.prototype.relogin = function relogin() {
    b.session_relogin(this._sp_session);
};

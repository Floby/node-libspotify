...
function Playlist (sp_playlist) {
    ...
    this._setupNativeCallbacks(); // We need to set up the callbacks first because we are using one of them to notify us when the object is ready rather than using a backoff isReady check
    SpObject.apply(this);
}

...
function Player (session) {
    ...
    this._session._sp_session.music_delivery = function(buffer) {
		// Readable.push returns whether or not we should push more data, so we're returning that value so the underlying code knows what to do
...
// We're inheriting from stream.Readable, which inherits from EventEmiiter so the track end event will still work

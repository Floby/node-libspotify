	    ...
	    if(this.isReady()) {
	        ...
	        bo.on('ready', function(number, delay) {
	        	//if(self._object_type == 'playlist'){
	        		//console.log('Checking BO playlist '  + i);
	        		//i++;
	        	//}
	            if(!self.isReady()) {
	                bo.backoff();
...
SpObject.getFromUrl = function getFromUrl(url) {
    ...
    if (this._object_type == 'playlist') {
		return new this(b[method_name](url, sp.Session.currentSession._sp_session)); // Because you need to pass session to create a playlist from a link
    } else {
		return new this(b[method_name](url));
    }
}
